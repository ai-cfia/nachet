...
async def create_test_inference(setup):
    ...
    with (
        patch("datastore.blob.azure_storage_api.get_blob") as mock_get_blob,
        patch(
            "datastore.blob.azure_storage_api.get_image_count"
        ) as mock_get_image_count,
    ):
        # Set up mocks
...
async def test_positive_feedback_successful(positive_feedback_setup):
    ...
    response = await setup["test_client"].post(
        ...
        json={"userId": setup["userId"], "inferenceId": inferenceId, "boxes": boxes},
    )
...
async def test_positive_feedback_missing_arguments_error(positive_feedback_setup):
    ...
    response = await setup["test_client"].post(
        ...
        json={"inferenceId": inferenceId, "boxes": boxes},
    )
    ...
    boxes.append(
        {
            ...
            "box": {"topX": 56, "topY": 36, "bottomX": 619, "bottomY": 302},
            "color": "#ED1C24",
            "overlapping": False,
            "overlappingIndices": [],
        }
