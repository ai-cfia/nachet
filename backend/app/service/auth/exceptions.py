...
class InvalidRequestWebSocket(WebSocketException):
    ...
    def __init__(self, detail: str) -> None:
        super().__init__(
            ...
            reason=str({"error": "invalid_request", "message": detail}),
        )
...
class UnauthorizedWebSocket(WebSocketException):
    ...
    def __init__(self, detail: str) -> None:
        super().__init__(
            code=status.WS_1008_POLICY_VIOLATION,
            reason=str({"error": "invalid_token", "message": detail}),
        )
...
class ForbiddenWebSocket(WebSocketException):
    ...
    def __init__(self, detail: str) -> None:
        super().__init__(
            code=status.WS_1008_POLICY_VIOLATION,
            reason=str({"error": "insufficient_scope", "message": detail}),
        )


#  --- start backwards-compatible code ---
def InvalidAuth(
    detail: str, request: HTTPConnection
...
def InvalidRequest(
    detail: str, request: HTTPConnection
) -> InvalidRequestHttp | InvalidRequestWebSocket:
...
def Forbidden(
    detail: str, request: HTTPConnection
) -> ForbiddenHttp | ForbiddenWebSocket:
