...
(
    ...
    original_ista_2025 text                                               not null
);
...
(
    ...
    deployment_platform text                     default 'on-prem'::text          not null,
    created_by          text                                                      not null,
    date_model_training timestamp with time zone                                  not null,
    description         text,
    job_name            text,
    dataset             text,
    artifacts_url       text,
...
comment on column model.job_name is 'Training job name';

comment on column model.dataset is 'training dataset id';
...
create unique index "nachet_0.0.11_pipeline_default"
    on pipeline (is_default)
...
alter table pipeline_model
    owner to nachet;

create table rbac_role
(
    id          uuid default gen_random_uuid() not null
        constraint rbac_role_pk
            primary key,
    name        text,
    description text,
    active      boolean
...
create table rbac_role_permission_resource
(
    role_id       uuid not null
        constraint rbac_role_permission_resource_rbac_role_id_fk
            references rbac_role,
    permission_id uuid not null
        constraint rbac_role_permission_resource_rbac_permission_id_fk
            references rbac_permission,
    resource_id   uuid not null
        constraint rbac_role_permission_resource_rbac_resource_id_fk
            references rbac_resource,
    constraint rbac_role_permission_resource_pk
        primary key (role_id, permission_id, resource_id)
);

alter table rbac_role_permission_resource
    owner to nachet;
...
(
    ...
    organization      uuid                     default '3b601416-9ee9-49d0-98f7-6890e598cb6e'::uuid not null
...
(
    id            uuid                     default gen_random_uuid() not null
        constraint picture_set_pkey
            primary key,
    user_id       uuid                                               not null
        constraint picture_set_owner_id_fkey
            references users,
    date_created  timestamp with time zone default CURRENT_TIMESTAMP not null,
    name          text,
    folder_prefix text,
...
(
    ...
    folder_id            uuid                                               not null
        constraint picture_picture_set_id_fkey
            references folder
            on delete cascade,
    active               boolean                  default true              not null,
    date_created         timestamp with time zone default CURRENT_TIMESTAMP not null,
    ...
    height               integer,
    description          text,
    size_on_disk         double precision
);

comment on column picture.single_species_image is 'for training, multiple seeds of the same species will be in the image, this value is specified by uploader, null if multi spp';
...
(
    id           uuid                     default gen_random_uuid() not null
        constraint inference_pkey
            primary key,
    raw_data     json                                               not null,
    picture_id   uuid                                               not null
        constraint inference_picture_id_fkey
            references picture
            on delete cascade,
    date_created timestamp with time zone default CURRENT_TIMESTAMP not null,
    user_id      uuid                                               not null
        constraint inference_user_id_fkey
            references users,
    pipeline_id  uuid,
    org_admin_id uuid
);
...
(
    ...
    inference_id     uuid                                               not null
        references annotation
            on delete cascade,
    top_id           uuid
        constraint object_seed_id_fk
            references seed,
    top_id_2         uuid,
    date_verified    timestamp with time zone,
    top_score        double precision,
    top_score_2      double precision,
    top_id_3         uuid,
    top_score_3      double precision,
    date_created     timestamp with time zone default CURRENT_TIMESTAMP not null,
    valid            boolean                  default true,
    box_update       boolean                  default false,
    species_update   boolean                  default false,
    user_id          uuid                                               not null,
    picture_id       uuid                                               not null
        constraint object_picture_id_fk
            references picture,
...
(
    user_id      uuid                                               not null,
    role_id      uuid                                               not null
        constraint rbac_user_role_rbac_role_id_fk
            references rbac_role,
    date_created timestamp with time zone default CURRENT_TIMESTAMP not null,
    active       boolean                  default true              not null,
...
create table device_lens
(
    id              uuid    default gen_random_uuid() not null
        constraint device_lens_pk
            primary key,
    name            text,
    description     text,
    active          boolean default true              not null,
...
alter table device_lens
    owner to nachet;

create table device_brand
(
    id          uuid    default gen_random_uuid() not null
        constraint device_brand_pk
            primary key,
    name        text,
    description text,
...
create view user_role_permissions (user_id, role_id, role_name, rp_id, permission_name, rr_id, resource_name) as
SELECT rbac_user_role.user_id,
       rbac_role.id   AS role_id,
       rbac_role.name AS role_name,
       rp.id          AS rp_id,
       rp.name        AS permission_name,
       rr.id          AS rr_id,
       rr.name        AS resource_name
FROM "nachet_0.2.0".rbac_user_role
         JOIN "nachet_0.2.0".rbac_role ON rbac_role.id = rbac_user_role.role_id
         JOIN "nachet_0.2.0".rbac_role_permission_resource rrpr ON rbac_role.id = rrpr.role_id
         JOIN "nachet_0.2.0".rbac_permission rp ON rrpr.permission_id = rp.id
         JOIN "nachet_0.2.0".rbac_resource rr ON rrpr.resource_id = rr.id;
