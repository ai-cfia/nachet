...
def validate_orm_classes_sync(db_url: str, debug: bool = False, logger=None):
    ...
    try:
        ...
        if logger:
            logger.info(
                "Using DB URL", hidden=not debug, url=db_url if debug else "[HIDDEN]"
            )
    ...
    try:
        ...
        if logger:
            logger.info(
                "Using async DB URL",
                hidden=not debug,
                url=db_url if debug else "[HIDDEN]",
            )
        ...
        if logger:
            logger.error(
                "Async ORM validation failed", error=str(e), error_type=type(e).__name__
            )
        return False
...
if __name__ == "__main__":
    ...
    logger.info("")
    async_valid = asyncio.run(
        validate_orm_classes_async(db_url=db_url, debug=DEBUG, logger=logger)
    )
    ...
    logger.info(
        f"Asynchronous validation: {'✅ PASSED' if async_valid else '❌ FAILED'}"
    )
    logger.info(
