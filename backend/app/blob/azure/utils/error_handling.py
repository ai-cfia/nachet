...
class ErrorHandler:
    ...
    def handle_service_errors(operation_name: str):
        ...
        def decorator(func: Callable) -> Callable:
            async def wrapper(*args, **kwargs) -> Any:
                ...
                except (ContainerNotFoundError, BlobNotFoundError, ConnectionError, BlobStorageError, InvalidConfigurationError):
                    # Re-raise our custom exceptions without wrapping
                    raise
                except ServiceRequestError as e:
                    raise ConnectionError(f"Failed to connect to Azure storage during {operation_name}: {str(e)}")
    ...
    @staticmethod
    async def check_blob_exists(client: BlobServiceClient, container: str, name: str) -> None:
        """
    ...
    def handle_resource_not_found(container: str, blob_name: str = None):
        ...
        """
        async def handler(client: BlobServiceClient, original_error: ResourceNotFoundError):
            try:
                container_client = client.get_container_client(container)
                if not container_client.exists():
                    raise ContainerNotFoundError(container, f"Container not found: {str(original_error)}")
                elif blob_name:
                    raise BlobNotFoundError(container, blob_name, f"Blob not found: {str(original_error)}")
                else:
                    raise ContainerNotFoundError(container, f"Container not found: {str(original_error)}")
            except (ContainerNotFoundError, BlobNotFoundError):
                raise
                ...
                if blob_name:
                    raise BlobNotFoundError(container, blob_name, f"Resource not found: {str(original_error)}")
                else:
                    raise ContainerNotFoundError(container, f"Resource not found: {str(original_error)}")

        return handler

    @staticmethod
    def handle_metadata_validation_errors(metadata: dict, operation_type: str = "metadata"):
        """
        ...
        if metadata:
            for key, value in metadata.items():
                if not isinstance(key, str) or not isinstance(value, str):
                    raise BlobStorageError(f"{operation_type.title()} keys and values must be strings")
                if not key.strip():
                    raise BlobStorageError(f"{operation_type.title()} keys cannot be empty")