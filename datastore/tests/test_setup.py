...
def setup_database():
    ...
    if not all([pg_user, pg_db, pg_password]):
        ...
        print("Optional: PG_TEST_HOST (default: 127.0.0.1), PG_TEST_PORT (default: 5432)")
        sys.exit(1)
    ...
    # Define file paths
    base_path = Path.cwd()
    schema_file = base_path / "nachet" / "db" / "bytebase" / f"schema_nachet_{schema_version}.sql"
    constants_file = base_path / "nachet" / "db" / "bytebase" / f"constants_nachet_{schema_version}.sql"
    ...
    schema_cmd = f'psql -h {pg_host} -p {pg_port} -U {pg_user} -d {pg_db} -f {schema_file}'
    run_command(schema_cmd, env=env)
    
    # Step 3: Apply constants file
    print("Applying constants data...")
    constants_cmd = f'psql -h {pg_host} -p {pg_port} -U {pg_user} -d {pg_db} -f {constants_file}'
    run_command(constants_cmd, env=env)
    
    # Step 4: Apply test data file
    print("Applying test data...")
    test_data_cmd = f'psql -h {pg_host} -p {pg_port} -U {pg_user} -d {pg_db} -f {test_data_file}'
