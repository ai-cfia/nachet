...
class EvaluationRuntimeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.args = argparse.Namespace(
            ...
            command=[
                sys.executable,
                "-c",
                f"from pathlib import Path; Path({str(report)!r}).write_text('{{}}')",
            ],
        )
    ...
    def test_publish_and_repeat_keep_one_child_and_completed_parent(self):
        ...
        self.assertEqual([child.info.run_id for child in children], [state["run_id"]])
        ...
        self.assertEqual(
            self.client.get_run(self.parent.info.run_id).info.status, "FINISHED"
        )
        self.assertEqual(
            [
                item.path
                for item in self.client.list_artifacts(state["run_id"], "evaluation")
            ],
    ...
    def test_cli_keeps_evaluator_logs_out_of_argo_json_result(self):
        ...
        self.assertEqual(
            self.client.get_run(result["mlflow_run_id"]).info.status, "FINISHED"
        )
    ...
    def test_failed_inference_is_not_marked_complete(self):
        with patch(
            "evaluation_runtime.subprocess.run",
            side_effect=subprocess.CalledProcessError(1, "evaluator"),
        ):
            with self.assertRaises(subprocess.CalledProcessError):
                execute(self.args, self.client)
        failed_state = json.loads(
            (self.args.output_dir / "evaluation.json").read_text()
        )
        self.assertFalse(failed_state["computed"])
        self.assertEqual(
            self.client.get_run(failed_state["run_id"]).info.status, "FAILED"
        )
        state = execute(self.args, self.client)
        self.assertTrue(state["computed"])
        self.assertTrue((self.args.output_dir / "failed-reports-1").exists())

    def test_failure_status_error_preserves_evaluator_error(self):
        error = subprocess.CalledProcessError(1, "evaluator")
        with (
            patch("evaluation_runtime.subprocess.run", side_effect=error),
            patch.object(
                self.client, "set_terminated", side_effect=OSError("MLflow unavailable")
            ),
        ):
    ...
    def test_empty_directory_is_not_a_completed_report(self):
        ...
        self.args.command = [
            sys.executable,
            "-c",
            f"from pathlib import Path; Path({str(directory)!r}).mkdir()",
        ]
    ...
    def test_changed_report_cannot_be_published_as_original_result(self):
        with patch.object(self.client, "log_artifacts", side_effect=OSError("offline")):
            with self.assertRaises(OSError):
                execute(self.args, self.client)
        (self.args.output_dir / "reports" / "metrics.json").write_text(
            '{"changed": true}'
        )
